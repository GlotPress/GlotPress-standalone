...
class GP_Test_Thing_Translation extends GP_UnitTestCase {
	...
	function test_propagate_across_projects_propagates_case_sensitiv() {
		add_filter( 'translations_to_other_projects_status', array( $this, '__string_status_current' ) );
		$set1 = $this->factory->translation_set->create_with_project_and_locale( array( 'locale' => 'bg' ), array( 'name' => 'project_one' ) );
		...
		$this->assertEquals( 1, count( $set2_current_translations ) );
		remove_filter( 'translations_to_other_projects_status', array( $this, '__string_status_current' ) );
	}
	...
	function test_propagate_across_projects_with_missing_permissions() {
		...
		add_filter( 'backpress_set_cookie', '__return_false' );
		$user->set_as_current();
		remove_filter( 'backpress_set_cookie', '__return_false' );
	...
	function test_propagate_across_projects_with_missing_permissions_does_not_create_duplicates() {
		...
		$translation1 = $this->factory->translation->create( array( 'translation_set_id' => $set1->id, 'original_id' => $original1->id, 'user_id' => $user->id, 'status' => 'current' ) );
		...
		$this->assertEquals( $set2_current_translations[0]->user_login, $user->user_login );
